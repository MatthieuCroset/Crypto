...
#Quel est le handle le plus court de cette liste ?
  list2 = list.sort_by {|x| x.length }
  #puts "Le handle le plus court est : #{list2[0]}"

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
compt=0

list2.each do |i|
  nb= i.length-1 #pour enlever un caractère @ ne compte pas
  nb5= nb==5
  if nb5== true
  compt= compt +1
  end
